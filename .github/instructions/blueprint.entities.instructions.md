...
- **`entity/`** - Base entity classes (inherit `SensusAnalyticsEntity` from `entity/base.py`)
...
**MUST inherit from:** `(PlatformEntity, SensusAnalyticsEntity)` - order matters for MRO
...
**Imports pattern:** `from homeassistant.components.PLATFORM import PlatformEntity, PlatformEntityDescription` + `from ..entity import SensusAnalyticsEntity`
