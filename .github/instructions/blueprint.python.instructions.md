...
- Files: `snake_case.py`
- Classes: `PascalCase` prefixed with `SensusAnalytics`
...
- Inherit from both platform entity and `SensusAnalyticsEntity` (order matters)
...
from custom_components.sensus_analytics.sensor import async_setup_entry
...
**Config entry data:** `entry_data: SensusAnalyticsData = hass.data[DOMAIN][entry.entry_id]`
